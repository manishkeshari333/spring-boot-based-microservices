...
//	    return new AuthTokenFilter(); // or whatever your filter class is
//	}

	@Bean
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(
				requests -> requests.requestMatchers("/api/v1/authmang/signin", "/api/v1/authmang/signup",
						"/api/v1/authmang/validate-token/**").permitAll().anyRequest().authenticated())
